...
class Sorter {
    ...
    calculatePriorityScore(email) {
        ...
        const isRevenue = REVENUE_KEYWORDS.some(keyword => subject.includes(keyword) || snippet.includes(keyword));
        if (isRevenue) {
        ...
        // 3. Check Urgency
        const isUrgent = URGENCY_KEYWORDS.some(keyword => subject.includes(keyword));
        if (isUrgent) {
