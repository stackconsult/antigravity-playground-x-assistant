...
async function main() {
    ...
    if (calendarIssues.length > 0) {
        ...
        calendarIssues.forEach(i => console.log(`   - [${i.severity}] ${i.title}: ${i.issue}`));
    } else {
    ...
    console.log('\n[AM] Processing Inbox...');
    const emails = [
        { subject: 'Invoice #1023', body: 'Please pay attached.', from: 'vendor@acme.com' },
        { subject: 'Quick Question', body: 'Can we meet?', from: 'employee@company.com' },
        { subject: 'Investment Opportunity', body: 'New startup...', from: 'founder@startup.com' }
    ];
